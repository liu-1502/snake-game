...
export const paletteVars = Object.fromEntries([
  ...
  /* Two tiles whose sizes share no useful factor, so the combined repeat is
     ...
     Each in two inks. The colour has to be baked into the data URI: an SVG
     used as a background-image is its own document and cannot pick up
     `currentColor` from the page. */
  ["--starfield-a", starField("#ffffff", 600, STARS_A)],
  ["--starfield-b", starField("#ffffff", 460, STARS_B)],
  ["--starfield-a-lit", starField("#000000", 600, STARS_A)],
  ["--starfield-b-lit", starField("#000000", 460, STARS_B)],
